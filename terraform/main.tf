  ...
  provisioner "remote-exec" {
    inline = [
      for network in each.value.networks : 
        "echo KERNEL==\"ens*\", SYSFS{address}==\"${module.nodes[each.value.id].networks[module.networks[network.id].id].mac_address}\", NAME=\"${module.networks[network.id].name}\" >> /etc/udev/rules.d/90_networkinterfaces.rules"      
    ]
  }
}
