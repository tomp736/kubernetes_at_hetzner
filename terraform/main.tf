...
resource "null_resource" "udev_network_interfaces" {
  ...
  provisioner "remote-exec" {
    inline = [
      for network in module.nodes[each.value.id].networks : 
        "echo KERNEL==\"ens*\", SYSFS{address}==\"${network.mac_address}\", NAME=\"${module.networks[network.network_id].name}\" >> /etc/udev/rules.d/90_networkinterfaces.rules"      
    ]
  }
}
